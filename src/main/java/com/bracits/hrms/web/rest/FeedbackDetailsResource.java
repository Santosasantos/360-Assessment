...
/**
 * REST controller for managing {@link com.bracits.hrms.domain.FeedbackDetails}.
 */
@RestController
...
public class FeedbackDetailsResource {
    ...
    public ResponseEntity<Double> fetchSelfRating(@RequestParam String pin, @RequestParam String feedbacksubname, @RequestParam Integer year) {
        log.debug("REST request to get rating for pin : {} and feedbacksubname : {}", pin, feedbacksubname,year);
        Optional<Double> rating = feedbackDetailsService.findSelfRating(pin, feedbacksubname, year);
        return ResponseUtil.wrapOrNotFound(rating);
    }
}
