...
import { EntityArrayResponseType, RatingService } from '../service/rating.service';
...
export class RatingComponent implements OnInit {
  ...
  trackId = (_index: number, item: IRating): number => this.ratingService.getRatingIdentifier(item);
  ...
  protected refineData(data: IRating[]): IRating[] {
    const { predicate, order } = this.sortState();
    return predicate && order ? data.sort(this.sortService.startSort({ predicate, order })) : data;
  }

  protected fillComponentAttributesFromResponseBody(data: IRating[] | null): IRating[] {
    return data ?? [];
  }
