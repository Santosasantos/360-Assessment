...
describe('FeedbackSubType Management Update Component', () => {
  ...
  describe('ngOnInit', () => {
    it('Should call FeedbackType query and add missing value', () => {
      ...
      const feedbacktypes: IFeedbackType = { id: 28285 };
      feedbackSubType.feedbacktypes = feedbacktypes;

      const feedbackTypeCollection: IFeedbackType[] = [{ id: 32170 }];
      jest.spyOn(feedbackTypeService, 'query').mockReturnValue(of(new HttpResponse({ body: feedbackTypeCollection })));
      const additionalFeedbackTypes = [feedbacktypes];
    ...
    it('Should update editForm', () => {
      const feedbackSubType: IFeedbackSubType = { id: 456 };
      const feedbacktypes: IFeedbackType = { id: 24339 };
      feedbackSubType.feedbacktypes = feedbacktypes;

      activatedRoute.data = of({ feedbackSubType });
      comp.ngOnInit();

      expect(comp.feedbackTypesSharedCollection).toContain(feedbacktypes);
