...
public class EmployeeServiceImpl implements EmployeeService {
    ...
    public List<Employee> getEligibleEmployees(ResponderCategory category, String currentUserPin) {
        Employee currentEmployee = employeeRepository.findByPin(currentUserPin)
            .orElseThrow(() -> new RuntimeException("Current employee not found"));

        switch (category) {
            case PEER:
                return employeeRepository.findPeers(currentEmployee.getId(),currentEmployee.getGrade(), currentEmployee.getFunctionalDesignation());
            case SUPERVISEE:
    ...
    public List<Employee> searchEmployeess(String term, ResponderCategory category, String currentUserPin) {
        ...
        return eligibleEmployees.stream()
            .filter(e -> e.getPin().contains(term) ||
